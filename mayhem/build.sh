...
# Build-time LSan-off hook (SPEC.md 6.1, mayhem/lsan_off.c), linked into every fuzz AND standalone
# binary. Replaces a forbidden runtime ASan default-options override; ASan use-after-free/overflow
# and UBSan stay fully on and halting. Named in prose, not by symbol: SPEC.md 6.2 item 15 bans the
# override symbol names anywhere under mayhem/, comments included.
LSAN_OBJ="$SRC/mayhem-build/lsan_off.o"
...
for h in $HARNESSES; do
  ...
  # libFuzzer target
  $CXX $CXXFLAGS "$obj" "$LSAN_OBJ" -Wno-unused-command-line-argument \
      $LDFLAGS_NS $LIB_FUZZING_ENGINE $LIBS_A $LIBS \
      -o "/mayhem/${h}_fuzzer"

  # standalone reproducer (no libFuzzer runtime)
  $CXX $CXXFLAGS "$SA_OBJ" "$obj" "$LSAN_OBJ" -Wno-unused-command-line-argument \
